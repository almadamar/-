...
async def kmr_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """فتح الإدارة بكلمة السر kmr"""
    if update.effective_user.id != OWNER_ID: return
    with open(DB_FILE, "r") as f: count = len(f.read().splitlines())
    
    kb = [
        [InlineKeyboardButton("📊 تحديث الإحصائيات", callback_data="adm_ref")],
        [InlineKeyboardButton("📂 نسخة احتياطية (Users)", callback_data="adm_bak")],
        [InlineKeyboardButton("📢 إذاعة (Broadcast)", callback_data="adm_bc")]
    ]
    await update.message.reply_text(f"🛠 **لوحة تحكم KMR**\n\n👤 المطور: أنمار\n👥 المشتركين: `{count}`", 
                                  reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')

async def handle_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالجة الأزرار مع حماية تداخل التحميل"""
    query = update.callback_query
    if query.from_user.id != OWNER_ID or "|" in query.data: return # حماية
    ...
    if query.data == "adm_ref":
        ...
        await query.message.reply_document(document=open(DB_FILE, 'rb'), caption="💾 نسخة مستخدميك.")
